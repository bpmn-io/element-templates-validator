...
function addWarningKeywords(ajv) {
  ...
  addDeprecationWarning(ajv, warnings);
}

/**
 Produces a "Value is deprecated" warning,
 or uses `deprecationWarning` message if provided alongside

 @param {Ajv} ajv
 @param {Array} warnings
 */
function addDeprecationWarning(ajv, warnings) {

  // as `deprecated` is a native ajv keyword, we need to remove it first to be able to overwrite
...
/**
 * Emit a warning object via AJV code generation to push warning objects into a shared
